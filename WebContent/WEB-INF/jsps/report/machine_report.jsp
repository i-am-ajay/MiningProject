...
<body class=mt-1>
	<div class="p-2 m-auto">
		...
		<div class="row">
			<h6 class="border-bottom mx-1 text-muted pb-2" id="form_title">Machine Report</h6>
		...
		<div  class="mt-3 bg-light p-1">
			<!--<form action="machine_report" method="POST">
			 <div class="form-row search">
		  	 <div class="form-group col-md-2">
			      <label for="machine_name" class="font-weight-bold">Machine Name<span class="text-danger">*</span></label>
			      <input class="form-control form-control-sm" id="m_name" placeholder="Machine Name" name="vehicle_no" title="Valid Number"/>
			    </div>
			     <div class="form-group col-md-2">
			      <label for="name" class="font-weight-bold">Owner<span class="text-danger">*</span></label>
			      <select class="form-control form-control-sm" id="client" placeholder="Choose Owner" name="client">
		      			<option value=""></option>
			...
			<table id="example" class="display compact cell-border" style="width:100%">
	        	<thead>
	            	<tr>
		                ...
		                <th>Owner</th>
		                <th>Machine Rate</th>
		                <th>Last 24hrs Unit</th>
		                <th>Last Fuel Unit</th>
		                <th>Creation Date</th>
		                <th>Fixed Hours</th>
		            </tr>
	        	</thead>
	        	<tbody>
        			...
        			<tr class="py-2 text-justify">
        				<td class="demo">${machine.name}</td>
	        			<td class="demo">${machine.vendorId.name}</td>
	        			<td class="demo">${machine.machineRate} </td>
	        			<td class="demo">${machine.last24HrsUnit}</td>
	        			<td class="demo">${machine.lastUnitForFuel}</td>
	        			<td id="address">${machine.entryDate}</td>
	        			<td id="fixe_hours">${machine.fixedHours}</td>
	        			</tr>
	        		</c:forEach>
	        	</tbody>
    		</table>
